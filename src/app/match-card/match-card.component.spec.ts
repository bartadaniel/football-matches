...
describe('MatchCardComponent', () => {
  ...
  it('displays competition name of match highlight', () => {
    const titleEl: HTMLElement = fixture.debugElement.query(
      By.css('.app-match-card__competition')
    ).nativeElement;
    expect(titleEl.textContent).toContain(mockMatchHighlight.competition.name);
  });

  it('displays thumbnail of match highlight', () => {
    const thumbnailEl: HTMLImageElement = fixture.debugElement.query(
      By.css('.app-match-card__thumbnail')
    ).nativeElement;
